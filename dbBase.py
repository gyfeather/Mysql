...
class DB_BASE(object):
    ...
    def __init__(self, db_name):
        ...
        connect_str = r"mysql+pymysql://gaoyu:123456@127.0.0.1:3306/{0}?charset=utf8".format(
            db_name)
...
class CBRC_Publication(DB_BASE):

    def __init__(self):
        db_name = 'check'
        table_name = 'cbrc_publication'
        super(CBRC_Publication, self).__init__(db_name)
        ...
        self.table_obj = Table(table_name, self.meta,
                               ...
                               Column('sub_index', Integer, primary_key=True),
                               Column('pubnumber', String(128), primary_key=True),
                               Column('person', String(128)),
                               Column('corpname', String(128)),
                               Column('delegator', String(128)),
                               Column('violatype', String(1024)),
                               Column('reason', String(1024)),
                               Column('punish_content', String(512)),
                               Column('whodid', String(128)),
                               Column('update_date', Integer),
                               Column('amount', Float)
                               )
...
if __name__ == '__main__':
    # print 'hello world'
    # dbapi = DB_BASE('check')
    # df = pd.read_sql('test',dbapi.engine)
    # print df.head()
    api = CBRC_Publication()
    api.create_table()
    # print 'done'